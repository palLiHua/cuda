...
int main(int argc, char **argv)
{
    ...
    CHECK(cudaMalloc((int**)&d_shared_var, sizeof(int)));
    CHECK(cudaMalloc((int**)&d_values_read_atomic, N * sizeof(int)));
    CHECK(cudaMalloc((int**)&d_values_read_unsafe, N * sizeof(int)));
    ...
    printf("  Using atomic operations produced an output of %d\n",
           h_shared_var_atomic);
    printf("In total, %d runs using unsafe operations took %f s\n",
           runs, unsafe_mean_time);
    printf("  Using unsafe operations produced an output of %d\n",
           h_shared_var_unsafe);
